...
export class CreateMovimientoIngresoDto {
  private constructor(
    ...
    public readonly detalles: DetalleIngresoItem[]
  ) {}

  static create(object: {
    [key: string]: any;
  }): [{ [key: string]: string }?, CreateMovimientoIngresoDto?] {
    ...
    if (!Array.isArray(detalles) || detalles.length === 0) {
      errors.detalles =
        'Debe proporcionar al menos un detalle de ingreso (producto)';
    } else {
    ...
    return [
      undefined,
      new CreateMovimientoIngresoDto(numericUsuario, numericTotal, detalles),
    ];
  }
}
